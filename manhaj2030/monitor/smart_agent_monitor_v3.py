...
# === Timed Task Relay (auto-scheduling) ===
def schedule_task(delay, agent, details):
    def delayed_send():
        time.sleep(delay * 60)
        send_to(agent, {"event": "TIMED_EXECUTION", "details": details})
    threading.Thread(target=delayed_send, daemon=True).start()
# === End Timed Task Relay ===
...
def main():
    ...
    # --------------------------------------------------
    print("🧭 Timed Task Relay activated inside smart_agent_monitor_v3.py")
    ...
    while True:
        ...
        if pos < len(lines):
            for i in range(pos, len(lines)):
                ...
                # Handle timed tasks
                if evt.get('type') == 'timed_task':
                    delay = evt.get('delay_minutes', 5)
                    agent_target = evt.get('to')
                    details = evt.get('details', {})
                    if agent_target:
                        schedule_task(delay, agent_target, details)
                    continue
                tgt = evt.get("to", "all")
